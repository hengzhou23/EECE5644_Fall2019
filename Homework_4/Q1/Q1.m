...
[r, g, b, n_row, n_col, normalized_bird_data] = image_loader(bird_raw_data);

plane_raw_data = imread('3096_colorPlane.jpg');
[r, g, b, n_row, n_col, normalized_plane_data] = image_loader(plane_raw_data);

% K values: [2 3 4 5]
for K = 2:5
    image_classifier(bird_raw_data, normalized_bird_data, n_row, n_col, K);
    image_classifier(plane_raw_data, normalized_plane_data, n_row, n_col, K);
end


function [] = image_classifier(raw_data, normalized_data, n_row, n_col, K)
    ...
    % Generate images 
    kmean_img = image_generator(kmeans_data, n_col, n_row);
    seg_img = labeloverlay(raw_data, label); % Overlay the original image with labeled matrices
    gmm_img = image_generator(gmm_data, n_col, n_row);
